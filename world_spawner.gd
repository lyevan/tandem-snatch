...
@export var module_length: float = 2.0  # Your measured mesh length
@export var road_speed: float = 5     # Controlled centrally here now
...
func _process(delta: float) -> void:
	...
	for child in get_children():
		child.position.z += road_speed * delta
